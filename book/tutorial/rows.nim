...
This is the SQL query generated by this `select` call:
    ...
    WHERE User.email = ? <- ['{customerBar.user.email}']
...
Since customer references a user, to update a customer, we also need to update its user. Norm handles that automatically by generating two queries:
    ...
    UPDATE "User" SET email = ? WHERE id = 2 <- @['{customerBar.user.email}']
    UPDATE "Customer" SET name = ?, user = ? WHERE id = 3 <- @['{get customerBar.name}', 2]
...
For each object in `customersFoo`, a pair of queries are generated:
    ...
    UPDATE "User" SET email = ? WHERE id = 1 <- @['{customersFoo[0].user.email}']
    UPDATE "Customer" SET name = ?, user = ? WHERE id = 1 <- @['{get customersFoo[0].name}', 1]
    UPDATE "User" SET email = ? WHERE id = 1 <- @['{customersFoo[1].user.email}']
    UPDATE "Customer" SET name = ?, user = ? WHERE id = 2 <- @['{get customersFoo[1].name}', 1]
